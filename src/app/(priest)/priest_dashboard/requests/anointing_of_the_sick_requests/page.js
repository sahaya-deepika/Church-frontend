...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllAnointingOfTheSickRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/aos_requests");
      ...
      setRequests(
        sorted.map((req) => ({ ...req, priest_response: req.priest_response ?? "" }))
      );
    } catch (err) {
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/aos_requests/${req._id}`, {
        status: decision,
...
// ─── request card ─────────────────────────────────────────────────────────────
function AnointingOfTheSickRequestCard({ req, submitting, onResponseChange, onApprove, onReject }) {
  ...
  return (
    <div
      className={`bg-white rounded-2xl border shadow-sm transition-all duration-200 overflow-hidden
        ...
        ${!isActedOn && req.is_urgent ? "ring-1 ring-red-200" : ""}`}
    >
      {/* Card top bar */}
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">Anointing of the Sick Request</span>
        </div>
        <div className="flex items-center gap-2">
          {req.is_urgent && (
            <span className="flex items-center gap-1 text-[11px] uppercase tracking-widest font-semibold border rounded-full px-3 py-0.5 bg-red-50 text-red-600 border-red-200">
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<User className="w-4 h-4" />} label="Patient">
          <span>{req.patient_name ?? "—"}</span>
          {req.relationship_to_patient && (
            <span className="flex items-center gap-1 text-gray-400 text-xs mt-0.5">
              <Users className="w-3 h-3" /> {req.relationship_to_patient}
            </span>
          )}
        </InfoRow>

        <InfoRow icon={<Stethoscope className="w-4 h-4" />} label="Condition">
          <span>{req.patient_condition ?? "—"}</span>
        </InfoRow>

        <InfoRow icon={<Phone className="w-4 h-4" />} label="Contact Number">
          <span>{req.contact_number ?? "—"}</span>
        </InfoRow>

        <InfoRow icon={<MapPin className="w-4 h-4" />} label="Patient's Address">
          <span>{req.patient_address ?? "—"}</span>
        </InfoRow>

        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred Visit date">
          <span>{req.preferred_visit_date_and_time ? safeFormat(req.preferred_visit_date_and_time) : "Not specified"}</span>
        </InfoRow>
      </div>
      ...
      {!isActedOn && (
        <div className="px-5 pb-5 border-t border-[#0F2A4A]/5 pt-4">
          ...
          <textarea
            rows={2}
            placeholder="Enter a message for the family — e.g. confirmed visit time, instructions, or reason for rejection…"
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All Anointing of the Sick requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllAnointingOfTheSickRequestsPage;