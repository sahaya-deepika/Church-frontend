...
// Build a usable image URL from the API's file_path (handles Windows-style backslashes)
...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllMassPrayerRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/mass_prayers");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/mass_prayer_requests/${req._id}`, {
        status: decision,
...
// ─── request card ─────────────────────────────────────────────────────────────
function MassPrayerRequestCard({ req, submitting, onResponseChange, onApprove, onReject }) {
  const isActedOn = req.status && req.status !== "pending";
  const isLoading = !!submitting;

  const [imageError, setImageError] = useState(false);
  const imageUrl = getMassPrayerImageUrl(req.mass_prayer_image);
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-amber-50 flex items-center justify-center">
            <Church className="w-4 h-4 text-amber-500" strokeWidth={1.5} />
          </div>
          <span className="text-white text-sm font-medium">Mass Prayer Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred Mass Prayer Date">
          <span>{safeFormat(req.mass_prayer_date_and_time)}</span>
        </InfoRow>

        <InfoRow icon={<Wallet className="w-4 h-4" />} label="Payment">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="capitalize">{payment.payment_mode ?? "—"}</span>
            {payment.amount != null && (
              <span className="text-gray-400 text-xs">₹{payment.amount}</span>
            )}
            {payment.payment_mode === "online" && payment.payment_status && (
              <span className={`text-[10px] uppercase tracking-wide font-semibold border rounded-full px-2 py-0.5 ${PAYMENT_STATUS_STYLES[payment.payment_status] ?? PAYMENT_STATUS_STYLES.pending}`}>
        ...
        <InfoRow icon={<ImageIcon className="w-4 h-4" />} label="Mass Prayer Image">
          {imageUrl && !imageError ? (
            <a
              href={imageUrl}
              target="_blank"
            ...
            >
              <img
                ...
                className="w-14 h-14 object-cover rounded-md border border-[#0F2A4A]/10"
              />
              <ExternalLink className="w-3 h-3 text-[#0F2A4A]/30 group-hover:text-[#C9A84C] transition-colors" />
            </a>
          ) : (
      ...
      {!isActedOn && (
        <div className="px-5 pb-5 border-t border-[#0F2A4A]/5 pt-4">
          ...
          <textarea
            rows={2}
            placeholder="Enter a message for the requester — e.g. confirmed date, instructions, or reason for rejection…"
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All mass prayer requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllMassPrayerRequestsPage;