...
// ─── main page ────────────────────────────────────────────────────────────────
const ViewAllBaptismRequestsPage = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiGet("/priest/baptism_requests");
  ...
  const handleReview = async (req, decision) => {
    ...
    try {
      const res = await apiPost(`/priest/baptism_requests/${req._id}`, {
        status: decision,
...
// ─── request card ─────────────────────────────────────────────────────────────
function BaptismRequestCard({ req, submitting, onResponseChange, onApprove, onReject }) {
    ...
    >
      ...
      <div className="bg-[#0F2A4A] px-5 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-white text-sm font-medium">Baptism Request</span>
        </div>
        {req.status && (
      ...
      <div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <InfoRow icon={<Baby className="w-4 h-4" />} label="Child">
          <span>{req.child_name ?? "—"}</span>
          {req.child_date_of_birth && (
            <span className="text-xs text-gray-400 mt-0.5">
              Born {safeFormat(req.child_date_of_birth, "dd MMM yyyy")}
            </span>
          )}
        </InfoRow>

        <InfoRow icon={<CalendarDays className="w-4 h-4" />} label="Preferred baptism date">
          <span>{safeFormat(req.preferred_baptism_date_and_time)}</span>
        </InfoRow>
      </div>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-gray-400">All baptism requests have been reviewed.</p>
    </div>
  );
}

export default ViewAllBaptismRequestsPage;